...
export default function EditProductPage() {
  ...
  useEffect(() => {
    if (productData) {
      form.reset({
        ...
        collectionId: productData.collections?.[0]?.id || "none",
        newCollection: "",
  ...
  const onSubmit = form.handleSubmit(async (values) => {
    ...
    const payload = {
      ...
      collectionIds: values.collectionId && values.collectionId !== "none" ? [values.collectionId] : [],
    ...
    if (!response.ok) {
      ...
      let responsePayload: { error?: string; issues?: { fieldErrors?: Record<string, string[]> } } | null = null;
      try {
      ...
      const fieldIssues = responsePayload?.issues?.fieldErrors
        ? Object.entries(responsePayload.issues.fieldErrors)
  ...
  if (isLoadingProduct) {
      return (
          <AdminShell>
              <div className="flex justify-center items-center h-64 text-forest/60">
    ...
    <AdminShell>
      <form onSubmit={onSubmit} className="space-y-6">
        <PageHeader
          eyebrow="Products"
          title="Edit Product"
          subtitle="Modify your existing product details, inventory, and variants."
          actions={
